...
export default function FriendsScreen() {
  ...
  const friendsWithStatus = useMemo(
    () => friends.map((friend) => ({ friend, status: mockStatusFor(friend.id) })),
    [friends]
  );

  const inTownThisWeek = useMemo(
    () => friendsWithStatus.filter((item) => item.status.state === 'in_town').length,
    [friendsWithStatus]
  );

  const filtered = useMemo(() => {
    const query = searchQuery.trim().toLowerCase();
    if (!query) return friendsWithStatus;
        ...
        {showEmptyState ? (
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyBody}>
              Add friends to see when they're in town and start planning together.
            </Text>
            <Button
